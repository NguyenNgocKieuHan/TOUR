...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($uploadOk) {
        $sql = "INSERT INTO destination (DESTINATIONID,DISTRICTID , TOURID, DESTINATIONNAME, IMAGE) 
                VALUES (?, ?, ?, ?, ?)";

        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, 'iiiss', $next_id, $district_id, $tour_id, $destination_name, $image_data);
